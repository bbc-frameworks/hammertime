...
function connectSockets (total, dataUrl, verbose, clientClass, onconnect) {
    ...
    setInterval(function () {
        ...
        connectorCalled = connections = disconnections = errors = 0;
    }, 2000);
    ...
    var connector = function (n) {
        ...
        socket.on('disconnect', function (e) {
            if (disconnected) {
//                sys.error('disconnected socket disconnected');
            }
            disconnected = true;
            connected--;
            disconnections++;
...
//                sys.debug('post response finished');
            });
        });
        request.end();
//        sys.debug('sending post request');
    };
    callback();
    ...
    for (var i in stats) {
        if (false && verbose) {
            sys.puts(
                ...
                ', connector called: : ' + stats[i].connectorCalled +
        ...
        connected       += stats[i].connected;
        connections     += stats[i].connections;
        connectorCalled += stats[i].connectorCalled;
        disconnections  += stats[i].disconnections;
        errors          += stats[i].errors;
    }
    sys.puts(
        ...
        'connected: ' + connected +
        ', connections: ' + connections +
        ', connector called: ' + connectorCalled +
        ', disconnections: ' + disconnections +
        ', errors: ' + errors
    );
}
...
function handleConnectionStats (stats, message) {
    ...
    stats[message.pid].connectorCalled += message.connectorCalled;
